<x-layouts.kpps :title="$title">
    ...
    <script>
        ...
        async function performCheckDpt() {
            ...
            try {
                const response = await fetch("{{ route('kpps.scanner.search-dpt') }}", {
                     method: 'POST',
                     headers: {
        ...
        async function handleBack() {
            if (isScanning && html5QrCode) {
                ...
                window.location.href = "{{ route('kpps.dashboard') }}";
            }
        }
        ...
        async function checkManualCode() {
            ...
            try {
                const response = await fetch("{{ route('kpps.scanner.check-access') }}", {
                     method: 'POST',
        ...
        async function submitManualValidation() {
            ...
            try {
                const response = await fetch("{{ route('kpps.scanner.verify-manual') }}", {
                     method: 'POST',
                     headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
        ...
        async function processVerification(code) {
            ...
            elLoading.classList.add('flex');
             ...
             try {
                 const response = await fetch("{{ route('kpps.scanner.verify') }}", {
                     method: 'POST',
                     headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
