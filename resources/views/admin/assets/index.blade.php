<x-layouts.admin title="File Manager">
    <div class="space-y-6 max-w-7xl mx-auto" x-data="{ 
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <nav class="flex items-center gap-2 text-sm text-slate-500 overflow-x-auto whitespace-nowrap scrollbar-hide py-2">
                ...
                @if($currentFolder)
                    @foreach($currentFolder->path as $path)
                        ...
                        <a href="{{ route('admin.assets.index', ['folder' => $path->id]) }}" 
            ...
            <div class="flex items-center gap-3">
                <div class="bg-slate-100 p-1 rounded-lg border border-slate-200 hidden sm:flex">
                    <a href="{{ route('admin.assets.index', array_merge(request()->query(), ['view' => 'grid'])) }}" 
                    ...
                    <a href="{{ route('admin.assets.index', array_merge(request()->query(), ['view' => 'list'])) }}" 
                       class="p-2 rounded-md transition-all {{ request('view') === 'list' ? 'bg-white shadow text-indigo-600' : 'text-slate-400 hover:text-slate-600' }}">
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach($folders as $folder)
                                    <tr class="group hover:bg-slate-50 transition-colors cursor-pointer" data-href="{{ route('admin.assets.index', ['folder' => $folder->id]) }}" onclick="window.location=this.dataset.href">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="relative" x-data="{ open: false }" @click.stop>
                                                ...
                                                <div x-show="open" class="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-10 py-1"
                                                    ...
                                                    <button data-rename-url="{{ route('admin.assets.folder.rename', $folder) }}" data-rename-name="{{ e($folder->name) }}" onclick="openRenameModal(this.dataset.renameUrl, this.dataset.renameName)" 
                                                    ...
                                                    <form action="{{ route('admin.assets.folder.destroy', $folder) }}" method="POST" onsubmit="return confirm('Hapus folder ini?')">
                                                        @csrf 
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            ...
                                            <div class="relative" x-data="{ open: false }" @click.stop>
                                                ...
                                                <div x-show="open" class="absolute right-0 top-6 w-32 bg-white rounded-lg shadow-xl border border-slate-100 z-50 py-1" style="display: none;">
                                                    ...
                                                    <button data-rename-url="{{ route('admin.assets.rename', $asset) }}" data-rename-name="{{ e($asset->name) }}" onclick="openRenameModal(this.dataset.renameUrl, this.dataset.renameName)" class="w-full text-left px-4 py-2 text-xs text-slate-600 hover:bg-slate-50 flex items-center gap-2">Rename</button>
                                                    <form action="{{ route('admin.assets.destroy', $asset) }}" method="POST" onsubmit="return confirm('Hapus file?')">
                                                        @csrf
                                                        <input type="hidden" name="_method" value="DELETE">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mt-6">
                        @foreach($folders as $folder)
                            <div class="group relative p-2 rounded-2xl hover:bg-slate-50 transition-all cursor-pointer"
                                 data-href="{{ route('admin.assets.index', ['folder' => $folder->id]) }}" onclick="window.location=this.dataset.href">
                                <!-- Folder Thumbnail -->
                                    ...
                                    <div class="absolute top-2 right-2" onclick="event.stopPropagation()">
                                        <div class="relative" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" class="absolute right-0 top-8 w-32 bg-white rounded-lg shadow-xl border border-slate-100 z-50 py-1" style="display: none;">
                                                <button data-rename-url="{{ route('admin.assets.folder.rename', $folder) }}" data-rename-name="{{ e($folder->name) }}" onclick="openRenameModal(this.dataset.renameUrl, this.dataset.renameName)" class="w-full text-left px-3 py-2 text-xs text-slate-600 hover:bg-slate-50">Rename</button>
                                                <form action="{{ route('admin.assets.folder.destroy', $folder) }}" method="POST" onsubmit="return confirm('Hapus folder ini?')">
                                                    @csrf 
                                                    <input type="hidden" name="_method" value="DELETE">
                                ...
                                <div class="aspect-[4/3] w-full rounded-xl overflow-hidden bg-slate-100 relative mb-3 shadow-sm group-hover:shadow-md transition-all">
                                    ...
                                    <div class="absolute top-2 right-2" onclick="event.stopPropagation()">
                                        <div class="relative" x-data="{ open: false }">
                                            ...
                                            <!-- Dropdown Menu -->
                                            <div x-show="open" class="absolute right-0 top-8 w-32 bg-white rounded-lg shadow-xl border border-slate-100 z-50 py-1" style="display: none;">
                                                <button data-rename-url="{{ route('admin.assets.rename', $asset) }}" data-rename-name="{{ e($asset->name) }}" onclick="openRenameModal(this.dataset.renameUrl, this.dataset.renameName)" class="w-full text-left px-3 py-2 text-xs text-slate-600 hover:bg-slate-50 flex items-center gap-2">
                                                    Rename
                                                </button>
        ...
        <div x-show="newFolderModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
            ...
            <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl scale-100" @click.away="newFolderModal = false">
                ...
                <form action="{{ route('admin.assets.folder.store') }}" method="POST">
                    @csrf
        ...
        <!-- Rename Modal -->
        <div id="rename-modal" class="hidden fixed inset-0 z-[60] items-center justify-center bg-black/50 backdrop-blur-sm">
