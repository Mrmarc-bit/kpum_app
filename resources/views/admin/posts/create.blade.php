    ...
    <div class="max-w-[1400px] mx-auto min-h-screen bg-[#F9FAFB]">
        ...
        <div class="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 sm:px-6 lg:px-8 shadow-sm flex items-center justify-between h-16">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-xl font-bold text-slate-800">Blogger Mode <span class="text-slate-400 font-normal text-sm ml-2">Draft as {{ Auth::user()->name ?? 'Admin' }}</span></h1>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.posts.index') }}" class="text-sm font-bold text-slate-500 hover:text-red-500 px-4 py-2 hover:bg-red-50 rounded-lg transition-colors">Discard</a>
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-6">
            <form id="post-form" action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row gap-8 items-start" data-turbo="false">
                @csrf
