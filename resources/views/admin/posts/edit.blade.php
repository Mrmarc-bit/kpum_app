<x-layouts.admin title="Edit Berita">
    <div class="px-4 py-8 max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden" data-turbo="false">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-4 rounded-b-3xl">
                ...
                <div class="flex gap-4">
                    <a href="{{ route('admin.posts.index') }}" class="px-6 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-colors">Batal</a>
                    <button type="submit" class="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-500/20 transition-all hover:-translate-y-0.5 min-w-[140px]">
                        Perbarui
                    </button>
                </div>
            </div>
