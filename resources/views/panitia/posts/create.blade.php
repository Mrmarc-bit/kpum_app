    ...
    <div class="max-w-[1400px] mx-auto min-h-screen bg-[#F9FAFB]">
        ...
        <div class="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 sm:px-6 lg:px-8 shadow-sm flex items-center justify-between h-16">
            <div class="flex items-center gap-4">
                <a href="{{ route('panitia.posts.index') }}" class="text-slate-500 hover:text-slate-800 transition-colors p-2 -ml-2 rounded-full hover:bg-slate-100">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                </a>
                <h1 class="text-xl font-bold text-slate-800">Blogger Mode <span class="text-slate-400 font-normal text-sm ml-2">Draft as {{ Auth::user()->name ?? 'Panitia' }}</span></h1>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('panitia.posts.index') }}" class="text-sm font-bold text-slate-500 hover:text-red-500 px-4 py-2 hover:bg-red-50 rounded-lg transition-colors">Discard</a>
                <button type="button" onclick="document.getElementById('post-form').submit()" class="px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold rounded-full shadow-md shadow-purple-500/20 transition-all flex items-center gap-2">
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-6">
            <form id="post-form" action="{{ route('panitia.posts.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row gap-8 items-start" data-turbo="false">
                @csrf
                ...
                <div class="w-full lg:w-3/4 flex flex-col gap-4">
                    ...
                    <!-- Title Input -->
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-2 overflow-hidden focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-500 transition-all min-h-[500px]">
                ...
                <div class="w-full lg:w-1/4 flex flex-col gap-4 lg:sticky lg:top-24">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100">
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            <select name="category" id="category" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 font-medium focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3" maxlength="250"
                                      class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 font-medium focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all placeholder:text-slate-400 resize-none"
                                      placeholder="Brief summary for UI cards...">{{ old('excerpt') }}</textarea>
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            <select name="is_published" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 font-medium focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                                <option value="1" {{ old('is_published') === '1' ? 'selected' : '' }}>Published (Live)</option>
